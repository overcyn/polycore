...
- (LYCollectionViewBehaviorOutput *)getAttributes:(LYCollectionViewBehaviorInput *)input {
    ...
    for (NSInteger i = 0; i < [[input collectionView] numberOfItemsInSection:[input section]]; i++) {
        ...
        CGSize size = [(id)[[input collectionView] delegate] collectionView:[input collectionView] layout:[input layout] sizeForItemAtIndexPath:indexPath insets:_insets];
...
- (LYCollectionViewBehaviorOutput *)getAttributes:(LYCollectionViewBehaviorInput *)input {
    ...
    CGSize size = [(id)[[input collectionView] delegate] collectionView:[input collectionView] layout:[input layout] sizeForItemAtIndexPath:indexPath insets:UIEdgeInsetsZero];
...
- (LYCollectionViewBehaviorOutput *)getAttributes:(LYCollectionViewBehaviorInput *)input {
    NSIndexPath *indexPath = [NSIndexPath indexPathForItem:0 inSection:[input section]];
    _scrollIndicatorInsets = [input scrollIndicatorInsets];
    CGSize size = [(id)[[input collectionView] delegate] collectionView:[input collectionView] layout:[input layout] sizeForItemAtIndexPath:indexPath insets:UIEdgeInsetsZero];
    
    LYCollectionViewLayoutAttributes *attr = [LYCollectionViewLayoutAttributes layoutAttributesForCellWithIndexPath:indexPath];
    [attr setFrame:CGRectMake(0, [input scrollIndicatorInsets].top, size.width, size.height)];
...
- (void)updateAttributes:(NSArray *)attributes withInput:(LYCollectionViewBehaviorInput *)input {
    ...
    LYCollectionViewLayoutAttributes *attr = attributes[0];
    CGRect f = [attr frame];
    f.origin.y = contentOffset.y + _scrollIndicatorInsets.top;
    [attr setFrame:f];
}
...
- (void)updateAttributes:(NSArray *)attributes withInput:(LYCollectionViewBehaviorInput *)input {
    UIEdgeInsets adjustedContentInset = [[input collectionView] adjustedContentInset];
    UIEdgeInsets contentInset = [[input collectionView] contentInset];
    CGPoint contentOffset = [[input collectionView] contentOffset];
    
    // Update the y origin to pin at the top
    LYCollectionViewLayoutAttributes *attr = attributes[0];
    CGRect f = [attr frame];
    f.origin.y = contentOffset.y + adjustedContentInset.top - contentInset.top;
    [attr setFrame:f];
    
    if (_contentOffsetDidChange) {
        _contentOffsetDidChange(contentOffset);
    }
}
...
@implementation LYHeaderShadowBehavior

- (LYCollectionViewBehaviorOutput *)getAttributes:(LYCollectionViewBehaviorInput *)input {
    NSMutableDictionary *context = [input context];
    NSNumber *exists = context[@"HeaderShadow"];
    context[@"HeaderShadow"] = @(true);
    
    NSIndexPath *indexPath = [NSIndexPath indexPathForItem:0 inSection:[input section]];
    CGSize size = [(id)[[input collectionView] delegate] collectionView:[input collectionView] layout:[input layout] sizeForItemAtIndexPath:indexPath insets:UIEdgeInsetsZero];
    LYCollectionViewLayoutAttributes *attr = [LYCollectionViewLayoutAttributes layoutAttributesForCellWithIndexPath:indexPath];
    [attr setFrame:CGRectMake(0, [input y], size.width, size.height)];
    [attr setZIndex:HEADER_Z - 1];
    [attr setHidden:exists != nil]; // Only show the first shadow.
    [attr setOffset:CGPointMake(0, [input y])];
